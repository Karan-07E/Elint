  ...
  useEffect(() => {
    if (filterType === 'all') {
      ...
      setFilteredUsers(users.filter(user => 
        user.role === 'accounts team' || (user.role === 'employee' && user.teamLeaderId?.role === 'accounts team')
      ));
    } else if (filterType === 'products') {
      setFilteredUsers(users.filter(user => 
        user.role === 'product team' || (user.role === 'employee' && user.teamLeaderId?.role === 'product team')
      ));
    }
  }, [filterType, users]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ name: "", email: "", password: "", role: "", teamLeaderId: "" });
        fetchUsers();
      ...
      <div className="ml-64 p-5 bg-slate-100 min-h-screen">
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-4">
          <div className="flex items-center gap-2">
            ...
            >
              Accounts Team ({users.filter(u => u.role === 'accounts team' || (u.role === 'employee' && u.teamLeaderId?.role === 'accounts team')).length})
            </button>
            <button
            ...
            >
              Product Team ({users.filter(u => u.role === 'product team' || (u.role === 'employee' && u.teamLeaderId?.role === 'product team')).length})
            </button>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., accounts team, employee, product team"
                  required
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
                  ...
                  >
                    ...
                    {teamLeaders.map(leader => (
                      <option key={leader._id} value={leader._id}>
                        {leader.name} ({leader.employeeId}) - {leader.role === 'accounts team' ? 'Accounts' : 'Products'}
                      </option>
                    ))}
                  </select>
                </div>
